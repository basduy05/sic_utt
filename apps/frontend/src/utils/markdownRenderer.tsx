...
import { BookOpen, ShieldAlert, CheckCircle, AlertTriangle, Info, Stethoscope, Sparkles, ChevronDown } from 'lucide-react';

export const ClinicalThinkingAccordion: React.FC<{ thinking: string }> = ({ thinking }) => {
  const [isOpen, setIsOpen] = useState(false);
  const lines = thinking.split('\n').map(l => l.trim()).filter(Boolean);

  return (
    <div className="my-2.5 rounded-2xl border border-teal-200/90 bg-gradient-to-br from-teal-50/40 via-white to-cyan-50/30 overflow-hidden shadow-xs transition-all">
      <button
        type="button"
        onClick={() => setIsOpen(!isOpen)}
        className="w-full px-3.5 py-2.5 flex items-center justify-between text-left hover:bg-teal-50/50 transition cursor-pointer select-none"
      >
        <div className="flex items-center space-x-2">
          <span className="p-1 rounded-lg bg-teal-100/80 text-teal-700">
            <Stethoscope className="w-3.5 h-3.5" />
          </span>
          <span className="text-xs font-bold text-teal-950 tracking-tight">
            Suy luận lâm sàng (Clinical Chain-of-Thought)
          </span>
          <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-teal-100/70 text-teal-800 border border-teal-200/60">
            Biện chứng y khoa
          </span>
        </div>
        <div className="flex items-center space-x-1.5 text-xs text-slate-400">
          <span className="text-[11px] font-medium text-teal-700/80">
            {isOpen ? 'Thu gọn' : 'Xem chi tiết'}
          </span>
          <ChevronDown
            className={`w-3.5 h-3.5 text-teal-600 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
          />
        </div>
      </button>

      {isOpen && (
        <div className="px-3.5 pb-3 pt-1 border-t border-teal-100/80 space-y-2 text-xs text-slate-700 animate-in fade-in duration-150">
          {lines.map((l, i) => {
              ...
              return (
                <div key={i} className="flex items-start space-x-2">
                  ...
                  <div className="leading-relaxed">
                    <strong className="text-teal-900 font-semibold">{label}:</strong>{' '}
            ...
            return (
              <div key={i} className="flex items-start space-x-2">
                <span className="w-1.5 h-1.5 rounded-full bg-teal-500 mt-1.5 flex-shrink-0" />
                <p className="leading-relaxed text-slate-600">{clean}</p>
              </div>
            );
          })}
        </div>
...
export function renderRichMarkdown(content: string, isRecordMode = false): React.ReactNode {
  ...
  // Tách khối suy luận lâm sàng <clinical_thinking>...</clinical_thinking> nếu có
  let thinkingContent: string | null = null;
  let mainText = content;
  const thinkingMatch = content.match(/<clinical_thinking>([\s\S]*?)<\/clinical_thinking>/);
  if (thinkingMatch) {
    thinkingContent = thinkingMatch[1].trim();
    mainText = content.replace(/<clinical_thinking>[\s\S]*?<\/clinical_thinking>/, '').trim();
  }
  ...
  return (
    <div className="space-y-0.5">
      {thinkingContent && <ClinicalThinkingAccordion thinking={thinkingContent} />}
      {renderedElements}
    </div>
  );
}
